...
def assign_pred_class(y_score):
    ...
    """
    # Amend the code here between ##
    pred_class = np.vectorize(_assign)(y_score)
    ################################
    return pred_class
...
def assign_true_class(y_true):
    ...
    """
    # Amend the code here between ##
    true_class = np.vectorize(_assign)(y_true)
    ################################
    return true_class
...
def print_model_perf(y_true, y_pred=None, y_score=None):
    ...
    text = ""
    # Amend the code here between ##
    text += "Model MSE = {:.4f}\n".format(metrics.mean_squared_error(y_true, y_score))
    ################################
    return text
...
def prepare_report():
    ...
    text_model_perf = print_model_perf(y_valid, y_score=y_score)
